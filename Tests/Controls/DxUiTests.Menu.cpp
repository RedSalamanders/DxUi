...
bool WaitForContextMenuPopupState(HWND popupHwnd,
                                  ...
                                  DxUi::ContextMenuPopupDebugState& outState,
                                  std::chrono::milliseconds timeout = std::chrono::milliseconds(800))
{
    const auto deadline = std::chrono::steady_clock::now() + timeout;
    do
    {
        if (DxUi::DebugGetContextMenuPopupState(popupHwnd, outState) && predicate(outState))
        {
            return true;
        }
                                     ...
                                     std::chrono::milliseconds timeout = std::chrono::milliseconds(800))
{
    ...
    do
    {
        if (DxUi::DebugGetContextMenuPopupItemRect(popupHwnd, itemIndex, outRectDip))
        {
            return true;
        }
...
bool WaitForContextMenuPopupItemLayout(HWND popupHwnd,
                                       size_t itemIndex,
                                       DxUi::ContextMenuPopupItemLayoutDebugState& outLayout,
                                       std::chrono::milliseconds timeout = std::chrono::milliseconds(800))
{
    const auto deadline = std::chrono::steady_clock::now() + timeout;
    do
    {
        if (DxUi::DebugGetContextMenuPopupItemLayout(popupHwnd, itemIndex, outLayout))
        {
            return true;
        }
...
bool WaitForContextMenuPopupItemPaint(HWND popupHwnd,
                                      size_t itemIndex,
                                      DxUi::ContextMenuPopupItemPaintDebugState& outPaint,
                                      std::chrono::milliseconds timeout = std::chrono::milliseconds(800))
{
    const auto deadline = std::chrono::steady_clock::now() + timeout;
    do
    {
        if (DxUi::DebugGetContextMenuPopupItemPaint(popupHwnd, itemIndex, outPaint))
        {
            return true;
        }
                                     ...
                                     std::chrono::milliseconds timeout = std::chrono::milliseconds(800))
{
    ...
    do
    {
        if (DxUi::DebugGetContextMenuPopupItemText(popupHwnd, itemIndex, outText))
        {
            return true;
        }
...
[[nodiscard]] uint8_t GetCapturePixelAlpha(const DxUi::WindowHostBitmapCapture& capture, UINT xPx, UINT yPx) noexcept
{
...
[[nodiscard]] uint32_t GetCapturePixelBgra(const DxUi::WindowHostBitmapCapture& capture, UINT xPx, UINT yPx) noexcept
{
    if (xPx >= capture.widthPx || yPx >= capture.heightPx)
    {
        return 0u;
...
[[nodiscard]] bool WaitForPendingSubmenuCloseTimerForMenuSuite(HWND popupHwnd, DxUi::ContextMenuPopupDebugState& outState)
{
    return WaitForContextMenuPopupState(
        popupHwnd, [](const DxUi::ContextMenuPopupDebugState& state) noexcept { return state.hoverTimerActive && state.hoverTimerPendingClose; }, outState);
}

[[nodiscard]] bool WaitForNoSubmenuHoverTimerForMenuSuite(HWND popupHwnd, DxUi::ContextMenuPopupDebugState& outState)
{
    return WaitForContextMenuPopupState(popupHwnd, [](const DxUi::ContextMenuPopupDebugState& state) noexcept { return ! state.hoverTimerActive; }, outState);
}

[[nodiscard]] bool FirePendingSubmenuHoverTimerForMenuSuite(HWND popupHwnd) noexcept
{
    return DxUi::DebugFireContextMenuPopupHoverTimer(popupHwnd);
}
                                                            ...
                                                            std::chrono::milliseconds timeout = std::chrono::milliseconds(1200))
{
    ...
    do
    {
        ...
        if (! timerPosted && DxUi::DebugGetContextMenuPopupState(popupHwnd, outState) && outState.hoverTimerActive && outState.hoverTimerPendingOpen &&
            outState.hoverTimerItemIndex.has_value() && outState.hoverTimerItemIndex.value() == itemIndex)
        {
                                                              ...
                                                              std::chrono::milliseconds timeout = std::chrono::milliseconds(1200))
{
    ...
    do
    {
        ...
        if (! timerPosted && DxUi::DebugGetContextMenuPopupState(popupHwnd, outState) && outState.hoverTimerActive && outState.hoverTimerPendingClose)
        {
            if (! FirePendingSubmenuHoverTimerForMenuSuite(popupHwnd))
            {
                return false;
...
DxUi::WindowHostBitmapCapture CropWindowHostBitmapCaptureForTest(const DxUi::WindowHostBitmapCapture& source, const RECT& cropRect)
{
...
[[nodiscard]] RECT FindOpaqueBoundsInCapture(const DxUi::WindowHostBitmapCapture& capture, uint8_t alphaThreshold = 8u) noexcept
{
...
[[nodiscard]] uint64_t ComputeAverageAdjacentRgbDelta(const DxUi::WindowHostBitmapCapture& capture, const RECT& sampleRect) noexcept
{
...
[[nodiscard]] uint64_t ComputeAverageAbsoluteRgbDeltaBetweenCaptures(const DxUi::WindowHostBitmapCapture& lhs,
                                                                     const DxUi::WindowHostBitmapCapture& rhs,
...
[[nodiscard]] uint64_t ComputeAverageAbsoluteRgbDeltaToColor(const DxUi::WindowHostBitmapCapture& capture,
...
[[nodiscard]] D2D1_COLOR_F ResolveExpectedAcrylicMenuSlabColor(const DxUi::ThemePalette& theme) noexcept
{
    return DxUi::BlendColor(theme.overlayBackground, theme.headerHovered, theme.dark ? 0.14f : 0.10f);
}
...
HWND FindOwnedContextMenuPopupWindowByFirstItemText(HWND ownerHwnd, std::wstring_view firstItemText)
{
    ...
    while ((popupHwnd = FindWindowExW(nullptr, popupHwnd, L"DxUi_ContextMenu", nullptr)) != nullptr)
    {
        ...
        if (DxUi::DebugGetContextMenuPopupItemText(popupHwnd, 0u, popupFirstItemText) && popupFirstItemText == firstItemText)
        {
            return popupHwnd;
        }
    }
...
std::shared_ptr<DxUi::MenuFlyoutItem::BitmapIcon> CreateSyntheticMenuBitmapIcon(UINT sizePx)
{
    if (sizePx == 0u)
    {
        return {};
    ...
    return std::make_shared<DxUi::MenuFlyoutItem::BitmapIcon>(std::move(bitmap), sizePx, sizePx);
}

DxUi::WindowHostBitmapCapture CaptureMenuPopupBitmapForTheme(const DxUi::ThemePalette& theme, const std::vector<DxUi::MenuFlyoutItem>& items)
{
    using namespace DxUi;
...
struct DestroyNativeMenuBarHostState final
{
    ...
    std::unique_ptr<DxUi::NativeMenuBarHost>* menuBarHost = nullptr;
    ...
    std::thread driver([&]
    {
        ...
        if (! popupHwnd)
        {
            ...
            if (HWND fallbackPopupHwnd = FindOwnedContextMenuPopupWindow(ownerWindow.Hwnd());
                fallbackPopupHwnd && DxUi::DebugGetContextMenuPopupItemText(fallbackPopupHwnd, 0u, popupFirstItemText))
            {
...
void TestContextMenuRootMinimumWidthUsesAnchorAndAllowsContentExpansion()
{
    ...
    AttachedHostWindow ownerWindow;
    SetWindowPos(ownerWindow.Hwnd(), nullptr, 180, 180, 420, 280, SWP_NOZORDER | SWP_NOACTIVATE);
    ShowWindow(ownerWindow.Hwnd(), SW_SHOWNOACTIVATE);
    ownerWindow.PumpMessages();

    constexpr float kAnchorWidthDip = 260.0f;
    ContextMenuSessionCallbacks callbacks{};
    callbacks.minRootWidthDip = kAnchorWidthDip;

    const auto measureMenuWidth = [&](std::wstring firstItemText, int commandId)
    {
        const std::vector<MenuFlyoutItem> items = {
        ...
        SendMessageW(popupHwnd, WM_KEYDOWN, VK_ESCAPE, 0);
        ownerWindow.PumpMessages();
        Require(callbackInvoked, "minimum-width context menu closes through Escape");
        Require(WaitForWindowDestroyed(popupHwnd), "minimum-width context menu window is destroyed after Escape");
        return popupState.visibleWidthDip;
    };
...
void TestContextMenuDpiRelayoutConstrainsOversizedContentToWorkArea()
{
    using namespace DxUi;

    AttachedHostWindow ownerWindow;
    std::vector<MenuFlyoutItem> items;
    constexpr size_t kItemCount = 128u;
    items.reserve(kItemCount);
    ...
    Require(popupHwnd != nullptr, "oversized DPI context menu popup appears");
    const auto dismissPopup = wil::scope_exit([&]() noexcept
    {
        if (IsWindow(popupHwnd) != FALSE)
        {
    ...
    ContextMenuPopupDebugState before{};
    Require(WaitForContextMenuPopupState(popupHwnd,
                                         [](const ContextMenuPopupDebugState& state) noexcept
    { return state.visibleWidthDip > 0.0f && state.visibleHeightDip > 0.0f && state.hasScrollbar; },
                                         before),
            "oversized DPI menu exposes bounded scrolling geometry");
    ...
    Require(WaitForContextMenuPopupState(popupHwnd,
                                         [targetDpi](const ContextMenuPopupDebugState& state) noexcept
    { return state.dpi == targetDpi && state.hasScrollbar && state.visibleWidthDip > 0.0f && state.visibleHeightDip > 0.0f; },
                                         after),
            "oversized DPI menu relayout completes with a scrolling viewport");
    Require(after.surfaceRectPx.left >= monitorInfo.rcWork.left && after.surfaceRectPx.top >= monitorInfo.rcWork.top &&
                after.surfaceRectPx.right <= monitorInfo.rcWork.right && after.surfaceRectPx.bottom <= monitorInfo.rcWork.bottom,
            "DPI relayout caps both oversized extents and position to the destination work area");

    SendMessageW(popupHwnd, WM_KEYDOWN, VK_END, 0);
    ContextMenuPopupDebugState endState{};
    Require(WaitForContextMenuPopupState(popupHwnd,
                                         [](const ContextMenuPopupDebugState& state) noexcept
    { return state.keyboardIndex == std::optional<size_t>{kItemCount - 1u}; },
                                         endState),
            "oversized DPI menu keeps the final command keyboard-reachable");
    D2D1_RECT_F lastItemRect{};
    Require(WaitForContextMenuPopupItemRect(popupHwnd, kItemCount - 1u, lastItemRect),
            "oversized DPI menu exposes the final command layout after End navigation");
    Require(lastItemRect.top >= endState.viewportRectDip.top - 0.5f && lastItemRect.bottom <= endState.viewportRectDip.bottom + 0.5f,
            "oversized DPI menu scrolls the final command fully into the bounded viewport");

    SendMessageW(popupHwnd, WM_KEYDOWN, VK_ESCAPE, 0);
    ownerWindow.PumpMessages();
    Require(callbackInvoked, "oversized DPI menu closes through Escape");
