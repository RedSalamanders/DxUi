...
separately reviewed CRT-exit lifetime fix (`claude/menu-exit-lifetime-fix`, the same destructor and
`MenuExitLifetime` suite as `d461095`). The localized-layout closeout already on main merged to
identical content. The described-menu paragraph now has its own section in the controls contract.
The gallery sheets and receipt take main's 30-control generation, so the described-menu example
must be regenerated on this tree. The MenuBar design-system guideline and static preview now show
a described entry; the design system still needs republishing.

The evidence archive is pruned from 1,593 files (18.2 MB) to 450 (2.1 MB): logs, receipt copies,
