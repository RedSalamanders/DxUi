...
NativeMenuBarHost::~NativeMenuBarHost()
{
    Detach();
}

void NativeMenuBarHost::SetTheme(const ThemePalette& theme) noexcept
{
    _theme = theme;
...
void NativeMenuBarHost::SetHeightDip(int heightDip) noexcept
{
...
void NativeMenuBarHost::SetOnEscape(std::function<bool()> onEscape)
{
    _host.SetOnEscape(std::move(onEscape));
}

[[nodiscard]] bool NativeMenuBarHost::Attach(HINSTANCE instance, HWND ownerWindow, HMENU menu, HWND commandTarget) noexcept
{
    ...
    HWND hwnd = CreateWindowExW(
        0, kNativeMenuBarHostWindowClassName, L"", WS_CHILD | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_TABSTOP, 0, 0, 1, 1, ownerWindow, nullptr, instance, this);
...
void NativeMenuBarHost::Detach() noexcept
{
    _menuBar          = nullptr;
    _menu             = nullptr;
...
void NativeMenuBarHost::SyncMenuModel() noexcept
{
    static_cast<void>(SyncMenuModelInternal(true));
}

void NativeMenuBarHost::UpdateLayout() noexcept
{
...
[[nodiscard]] int NativeMenuBarHost::GetVisibleHeightPx() const noexcept
{
...
[[nodiscard]] bool NativeMenuBarHost::FocusFirstItem() noexcept
{
    if (! SyncMenuModelInternal(true))
    {
        return false;
...
[[nodiscard]] bool NativeMenuBarHost::ActivateMnemonic(wchar_t mnemonic) noexcept
{
    if (! SyncMenuModelInternal(true))
    {
        return false;
...
[[nodiscard]] HWND NativeMenuBarHost::GetHwnd() const noexcept
{
    return _hwnd.get();
}

[[nodiscard]] bool NativeMenuBarHost::EnsureWindowClass(HINSTANCE instance) noexcept
{
    WNDCLASSW existing{};
...
void NativeMenuBarHost::CaptureFocusRestoreTarget() noexcept
{
    DxUi::CaptureFocusRestoreTarget(_ownerWindow, _hwnd.get(), _focusRestoreHwnd);
}

void NativeMenuBarHost::RestoreCapturedFocus() noexcept
{
    static_cast<void>(DxUi::RestoreCapturedFocus(_focusRestoreHwnd, _ownerWindow));
}

[[nodiscard]] bool NativeMenuBarHost::SyncMenuModelInternal(bool invokeRefresh) noexcept
{
    if (! _menuBar || ! _menu)
    {
        return false;
    ...
    menuBar->SetOnOpenItem([this](size_t index, POINT screenPoint, bool keyboardInvocation) noexcept { OpenPopup(index, screenPoint, keyboardInvocation); });
    _host.Invalidate();
    return true;
}

[[nodiscard]] std::optional<size_t> NativeMenuBarHost::HitTestScreenPoint(POINT screenPoint) const noexcept
{
    if (! _menuBar || ! _hwnd || IsWindow(_hwnd.get()) == FALSE)
    {
        return std::nullopt;
...
[[nodiscard]] std::optional<POINT> NativeMenuBarHost::GetItemAnchorScreenPoint(size_t index) const noexcept
{
    RECT itemRectPx{};
...
[[nodiscard]] std::optional<size_t> NativeMenuBarHost::FindNextEnabledItem(size_t currentIndex, bool forward) const noexcept
{
    if (! _menuBar)
    {
        return std::nullopt;
...
[[nodiscard]] std::optional<ContextMenuRootSwitchRequest> NativeMenuBarHost::BuildRootSwitchRequest(size_t index) noexcept
{
    if (! _menu || ! _menuBar)
    {
        return std::nullopt;
...
void NativeMenuBarHost::OpenPopup(size_t index, POINT screenPoint, bool keyboardInvocation) noexcept
{
...
LRESULT CALLBACK NativeMenuBarHost::WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) noexcept
{
