...
| Slider | Set range/value and handle `SetOnChange(SliderChange)` for preview, commit and cancel. The track is 12 DIP thick with a 48 DIP thumb; the full control bounds are the hit target, so keep height at least 48 DIP for touch. |
