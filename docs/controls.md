...
| Slider | Set range/value and handle `SetOnChange(SliderChange)` for preview, commit and cancel. Painted chrome is a 6 DIP track, a 14 DIP inner thumb (16 hover, 12 pressed) and a translucent halo (20 / 28 / 36 DIP). Pointer hit testing is an unpainted 48 DIP band; a press within 24 DIP of the thumb center drags, otherwise the track seeks. Hover, press and keyboard steps animate; `SetValue` snaps. |
