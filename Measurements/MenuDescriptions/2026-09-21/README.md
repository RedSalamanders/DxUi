...
## Archive pruning (2026-09-25)

When this work was rebased onto main, 1143 files (16.23 MB) were pruned from the 2026-09-21
and 2026-09-23 packets: every `*.log`, `*.receipt.txt` and `*.source.txt` file and the contents of
`raw/` directories, except files linked from a README. READMEs, comparisons, JSON reports, patches,
driver scripts and fixture inputs remain, and each affected README states its removed count. The
complete byte-exact archive, and the source commits these receipts cite, stay on
`codex/menu-description-layout` at `36a1c43`: keep or tag that branch before deleting it. Pruning
changes no measured value, threshold or conclusion.
